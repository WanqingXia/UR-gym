    ...
    def step(self, action: np.ndarray) -> Tuple[Dict[str, np.ndarray], float, bool, bool, Dict[str, Any]]:
        ...
        if self.spec.id == "UR5DynReach-v1":
            ...
            if np.linalg.norm(self.task.obstacle_end[:3] - self.sim.get_base_position("obstacle"),  axis=-1) > 0.05:
                time_duration = 1
                linear_velocity = (self.task.obstacle_end[:3] - self.task.obstacle_start[:3]) / time_duration
                # Calculating the relative rotation from start to end orientation
                rot_end = self.sim.euler_to_quaternion(self.task.obstacle_end[3:])
                rot_start = self.sim.euler_to_quaternion(self.task.obstacle_start[3:])
                relative_rotation = self.sim.get_quaternion_difference(rot_end, rot_start)
